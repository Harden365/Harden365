...
function AuditMenu() {
  ...
  $AuditMenu = CreateMenu -TenantName $TenantName -TenantEdition $TenantEdition -O365ATP $O365ATP -MenuTitle "HARDEN 365 - AUDIT" -MenuOptions @("Audit Microsoft Defender for O365 with ORCA", "Audit Administration Roles", "Audit Identity Users", "Audit Identity Applications", "Audit Autoforwarding", "Audit Mailbox Permissions", "Check DNS Records", "<- Return")
  switch ($AuditMenu) {
    0 {
      write-host $FrontStyle -ForegroundColor Red
    ...
    3 {
      write-host $FrontStyle -ForegroundColor Red
      write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; write-host("AUDIT IDENTITY APPLICATIONS") -ForegroundColor Red
      write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; Write-host ('Connected to Graph') -ForegroundColor Green

      $scriptFunctions = (Get-ChildItem function: | Where-Object { $_.source -match 'Harden365.Identity.Applications' })
      $scriptFunctions | ForEach-Object {
        Try { 
          & $_.Name -ErrorAction:SilentlyContinue | Out-Null
        }
...
function IdentityMenu() {
  ...
  switch ($IdentityMenu) {
    ...
    2 {
      write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; write-host("Do you want to create Conditionnal Access Standard (Y/N) : ") -NoNewline -ForegroundColor Yellow ; $QID2 = Read-Host
      if ($QID2 -eq 'Y') {
        write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; Write-host ('Check Emergency Accounts') -ForegroundColor Green
        ...
        {
            write-host $FrontStyle -ForegroundColor Red
            write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; write-host("HARDENING CONDITIONNAL ACCESS FOR ENTRA ID") -ForegroundColor Red
            write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; Write-host ('Connected to Graph') -ForegroundColor Green
            ...
            write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; Write-host ('All CA Standard created is disable by default') -ForegroundColor Green
            Read-Host -Prompt "Press Enter to return_"
        }
        else {
              write-host $(Get-Date -UFormat "%m-%d-%Y %T ") -NoNewline ; Write-host ('Please create Emergency Account first') -ForegroundColor Yellow
              Read-Host -Prompt "Press Enter to return_"
        }
        IdentityMenu -TenantName $TenantName -TenantEdition $TenantEdition -O365ATP $O365ATP
      }
